...
export function coerceMissing(input: unknown): MissingFilter {
  return input === "content" || input === "photos" || input === "title" || input === "desc" ? input : "all"
}

export function coerceSort(input: unknown): SortKey {
  return input === "stock_desc" || input === "price_asc" || input === "price_desc" ? input : "group"
}

export function getGroupLabel(r: AdminProductListRow): string {
  return r.group_name_en ?? r.group_name_ka ?? r.group_name ?? "—"
}
